...
function TechFeatureCard({
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute -inset-4 ${gradientFrom} ${gradientTo} bg-gradient-to-r rounded-3xl blur-xl opacity-20 group-hover:opacity-40 transition-all duration-700 ${
      ...
      <div className="relative bg-white/95 backdrop-blur-xl rounded-3xl p-8 shadow-2xl border border-white/30 group-hover:bg-white transition-all duration-500 group-hover:shadow-3xl group-hover:scale-105">
        ...
        {/* Title */}
        <h3
          className={`text-2xl lg:text-3xl font-black ${accentColor} mb-4 tracking-tight group-hover:scale-105 transition-transform duration-300`}
        >
          {title}
        </h3>
        ...
        {stats && (
          <div
            className={`${gradientFrom} ${gradientTo} bg-gradient-to-r rounded-2xl p-4 text-center`}
          >
            <div className="text-3xl font-black text-white mb-1">
              {stats.value !== undefined ? (
                <AnimatedNumber target={stats.value} suffix={stats.suffix} />
              ) : (
                <span>{stats.suffix}</span>
              )}
            </div>
        ...
        <div
          className={`mt-6 h-1 ${gradientFrom} ${gradientTo} bg-gradient-to-r rounded-full transform origin-left transition-all duration-500 ${
...
function TechHighlightBanner({ isVisible }: { isVisible: boolean }) {
  return (
    <div
      className={`relative overflow-hidden rounded-3xl transition-all duration-1500 ${
        ...
        <div className="flex flex-col lg:flex-row items-center justify-center gap-8 lg:gap-16">
          ...
          <div className="flex-1 text-white">
            ...
            <p className="text-xl lg:text-2xl text-white/90 leading-relaxed max-w-3xl">
              Proven high accuracy and open validation capabilities with
              super-fast conditional modelling
            </p>
          </div>
        </div>
      </div>
  ...
  const techFeatures = [
    {
      ...
      description:
        "Builds comprehensive simulations in minutes or hours, not weeks or months. Revolutionary speed meets enterprise-grade precision.",
      accentColor: "text-primary-900",
      gradientFrom: "from-purple-500",
      gradientTo: "to-indigo-600",
      stats: { suffix: "Fast", label: "Simulation Build Time" },
    },
    {
      icon: "/icons8-graph-100.png",
      title: "Shock Synthetic Data Engine",
      description:
        "Generates shock events data on multi-variant input factors with unprecedented accuracy and detail for comprehensive risk analysis.",
      accentColor: "text-primary-900",
      gradientFrom: "from-purple-500",
      gradientTo: "to-indigo-600",
    ...
    },
    {
      ...
      description:
        "Identifies many unknown variables and intelligently weights the best fit, uncovering hidden risk patterns in complex market scenarios.",
      accentColor: "text-primary-900",
      gradientFrom: "from-purple-500",
      gradientTo: "to-indigo-600",
      stats: { value: 250, suffix: "+", label: "Risk Variables" },
    },
  ];

  return (
    <section
      id="tech-showcase-section"
        ...
        <div className="text-center mb-16">
          ...
          >
            ...
            <p className="text-xl lg:text-2xl text-gray-700 leading-relaxed max-w-4xl mx-auto">
              Revolutionary technology stack powering the future of financial
              risk intelligence
            </p>
          </div>
        </div>
