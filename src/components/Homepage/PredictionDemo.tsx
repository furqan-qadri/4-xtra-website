...
// Scenario selector
function ScenarioSelector({ scenarios, activeScenario, onSelect }: { scenarios: any, activeScenario: number, onSelect: (index: number) => void }) {
  return (
...
export default function PredictionDemo({ isVisible }: { isVisible: boolean }) {
  ...
  return (
    <div className={`w-full max-w-4xl mx-auto space-y-6 transition-all duration-1000 delay-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
      
      {/* Scenario selector */}
      <div className="space-y-4">
        <h3 className="text-2xl font-semibold text-white text-center">Try a Live Scenario</h3>
        <ScenarioSelector 
          scenarios={scenarios}
      ...
      <div className="p-8 rounded-xl bg-white/5 backdrop-blur-sm border border-gray-600/20 space-y-6">
        ...
        <div className="p-4 rounded-lg bg-white/5 backdrop-blur-sm border border-gray-600/20">
          ...
          <div className="text-sm text-gray-300">Hedge funds, wealth managers, and institutional investors worldwide</div>
        </div>
      </div>
    </div>
  );
